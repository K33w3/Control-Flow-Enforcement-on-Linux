...
Clang drops the nocf_check attribute when function pointers are cast through the blurred barrier, and fails to emit notrack if the call is tail-call optimized. The harness generator bypasses this by applying the attribute to both the variable and the cast, dereferencing the pointer explicitly via (*p)(), and placing an empty inline assembly block after the call to prevent tail calls.
